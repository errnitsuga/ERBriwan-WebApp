...
const getRegistrationErrorMessage = (err: unknown) => {
  ...
  return "Failed to register responder.";
};

const ORGANIZATION_DB_VALUES = new Set(
  ORGANIZATION_OPTIONS.map((option) => option.value),
);

const ONLY_REGION = NCR_REGION;
const ONLY_CITY = "Quezon City";
...
export function ReceiverRegistration() {
  ...
  const barangayOptions = CITY_TO_BARANGAYS[ONLY_CITY] || [];
  ...
  const handleChange = (
    ...
    setFormData((prev) => {
      if (name === "region" || name === "city_municipality") {
        return prev;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.region !== ONLY_REGION) {
      setError("Only NCR is allowed for region.");
      return;
    }

    if (formData.city_municipality !== ONLY_CITY) {
      setError("Only Quezon City is allowed for city/municipality.");
      return;
    }

    const cityBarangays = CITY_TO_BARANGAYS[ONLY_CITY] || [];
    if (!cityBarangays.includes(formData.barangay)) {
      setError("Please select a valid barangay inside Quezon City.");
      return;
    }

    if (!ORGANIZATION_DB_VALUES.has(formData.organization)) {
    ...
    try {
      ...
      setLastRegisteredName(fullName);
      setIsSuccess(true);
          ...
          <div className="md:w-1/3 bg-[#1E3A8A] p-10 text-white relative overflow-hidden">
            <div className="relative z-10">
              ...
              <p className="text-blue-100 text-sm leading-relaxed opacity-90">
                Register a new official responder account to the ERBriwan
                system. Activated accounts will have access to the responder
                portal and receive real-time alerts.
              </p>
            ...
            {isSuccess ? (
              ...
              >
                ...
                <p className="text-gray-500 max-w-sm">
                  The responder account for{" "}
                  {lastRegisteredName || "the selected responder"} has been
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-center gap-2 p-4 bg-amber-50 rounded-xl border border-amber-100 text-amber-800">
                  <AlertCircle size={20} className="shrink-0" />
                  <p className="text-xs">
                    Please ensure all information is accurate. False
                    registrations may compromise emergency response
                    coordination.
                  </p>
                </div>
